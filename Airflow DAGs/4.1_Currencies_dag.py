...
load_cbr_xml_script = '''
curl https://www.cbr.ru/scripts/XML_daily.asp?date_req=01/11/2021 | iconv -f Windows-1251 -t UTF-8 > /tmp/currencies.xml
'''
...
def export_xml_to_csv_func():
    ...
    tree = ET.parse('/tmp/currencies.xml', parser=parser)
    root = tree.getroot()

    with open('/tmp/currencies.csv', 'w') as csv_file:
...
def load_csv_to_gp_func():
    ...
    pg_hook.copy_expert("COPY test_cbr FROM STDIN DELIMITER ','", '/tmp/currencies.csv')
